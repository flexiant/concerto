...
func cmdUpdate(c *cli.Context) {
	...
	jsonBytes, err := json.Marshal(v)
	utils.CheckError(err)
	err, res := webservice.Put(fmt.Sprintf("/v1/settings/saas_accounts/%s", c.String("id")), bytes.NewReader(jsonBytes))
...
func cmdDelete(c *cli.Context) {
	...
	err, res := webservice.Delete(fmt.Sprintf("/v1/settings/saas_accounts/%s", c.String("id")))
	utils.CheckError(err)
